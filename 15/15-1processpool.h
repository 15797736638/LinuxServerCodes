    ...
    : m_listenfd( listenfd ), m_process_number( process_number ), m_idx( -1 ), m_stop( false )
{
    ...
    for( int i = 0; i < process_number; ++i )
    {
        ...
        if( m_sub_process[i].m_pid > 0 )//pid>0，当前在父进程
        {
...
//父进程中m_ix（子进程在池中的信号）值为-1，子进程中m_idx值大于等于0，我们据此判断接下来要运行的是父进程代码还是子进程代码
template< typename T >
...
void processpool< T >::run_child()
{
    ...
    while( ! m_stop )
    {
        ...
        for ( int i = 0; i < number; i++ )
        {
            ...
            //如果是其他刻度数据，那么必然是客户请求到来，调用逻辑处理对象的process方法处理之
    ...
    users = NULL;
    close( pipefd );
    //close( m_listenfd );/*提醒：应该由m_listenfd的创建者来关闭这个文件描述符，即所谓的对象（比如
    /*一个文件描述符，又或者一段堆内存由哪个函数创建，就应该由哪个函数销毁）    */
    close( m_epollfd );
}
                ...
                else
                {
                    for( int i = 0; i < ret; ++i )
                    {
                        switch( signals[i] )
                        {
                            ...
                            case SIGTERM:
                            case SIGINT:
                            {/*如果父进程接收到终止信号，那么就杀死所有子进程，并等待它们全部结束。当然，通知子进程结束更好
                            的方法是向父、子进程之间的通信管道发送特殊数据，读者不妨自己实现之*/
